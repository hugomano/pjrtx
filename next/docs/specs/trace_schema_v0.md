...
in the new PjRTx architecture under `//next/pjrtx/...`.
...
boundaries. The compiler pass catalog in `//next/pjrtx/compiler` gives the broader
...
Profile event facts live under `//next/pjrtx/runtime/facts`. Compiler facts,
backend facts, runtime facts, and target facts are rendered together by
`//next/pjrtx/report`, so reports can join compile-time and runtime observations

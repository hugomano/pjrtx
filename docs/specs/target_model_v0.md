...
records, and target-local summary rendering live in `//pjrtx/target`. During the
`//pjrtx/core` migration, old names may be re-exported only as a compatibility
...
Profile event facts belong under `//pjrtx/runtime/facts`, while target
hardware facts remain under `//pjrtx/target`. Runtime tests only validate event

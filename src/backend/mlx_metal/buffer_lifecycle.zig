const std = @import("std");
const ir = @import("src/compiler/ir");

const buffer_handle = @import("buffer_handle.zig");
const encoding = @import("buffer_encoding.zig");
const mlx_call = @import("mlx_call.zig");

pub const Error = buffer_handle.Error;

/// Copies host bytes into a typed MLX/Metal buffer.
pub fn fromHost(comptime Buffer: type, device_local_hardware_id: i32, element_type: ir.BufferType, dims: []const i64, src: []const u8) Error!?Buffer {
    if (src.len == 0) return null;
    const dtype = encoding.dtype(element_type) orelse return error.UnsupportedElementType;
    return buffer_handle.wrap(Buffer, mlx_call.bufferFromHost(device_local_hardware_id, dtype, dims, src), error.BufferAllocationFailed);
}

/// Allocates a zero-initialized MLX/Metal buffer.
pub fn zeros(comptime Buffer: type, device_local_hardware_id: i32, element_type: ir.BufferType, dims: []const i64) Error!?Buffer {
    const dtype = encoding.dtype(element_type) orelse return error.UnsupportedElementType;
    return buffer_handle.wrap(Buffer, mlx_call.bufferZeros(device_local_hardware_id, dtype, dims), error.BufferAllocationFailed);
}

/// Clones this buffer into a new MLX/Metal buffer.
pub fn clone(src: anytype) Error!?@TypeOf(src) {
    return buffer_handle.wrap(@TypeOf(src), mlx_call.bufferClone(src.handle), error.CommandSubmissionFailed);
}

/// Creates a zero buffer with the same MLX/Metal shape and type as this buffer.
pub fn zeroLike(src: anytype) Error!?@TypeOf(src) {
    return buffer_handle.wrap(@TypeOf(src), mlx_call.bufferZeroLike(src.handle), error.CommandSubmissionFailed);
}

/// Forces this MLX/Metal buffer to evaluate.
pub fn eval(buffer: anytype) Error!void {
    if (!mlx_call.bufferEval(buffer.handle)) return error.CommandSubmissionFailed;
}

/// Forces all supplied MLX/Metal buffers to evaluate as a single backend call.
pub fn evalMany(buffers: anytype) Error!void {
    if (!mlx_call.bufferEvalMany(buffer_handle.rawHandles(buffers))) return error.CommandSubmissionFailed;
}

/// Copies this MLX/Metal buffer into host memory.
pub fn copyToHost(buffer: anytype, dst: []u8) Error!void {
    if (!mlx_call.bufferCopyToHost(buffer.handle, dst)) return error.BufferCopyFailed;
}

/// Reports whether this buffer still owns a host shadow allocation.
pub fn hasHostShadow(buffer: anytype) bool {
    return mlx_call.bufferHasHostShadow(buffer.handle);
}

/// Destroys this MLX/Metal buffer handle.
pub fn destroy(buffer: anytype) void {
    mlx_call.bufferDestroy(buffer.handle);
}

test "buffer handle wrapper preserves opaque identity" {
    const raw: *anyopaque = @ptrFromInt(@alignOf(usize));
    const Buffer = struct {
        handle: mlx_call.BufferHandle,

        fn fromHandle(handle: *anyopaque) @This() {
            return .{ .handle = @ptrCast(handle) };
        }

        fn toHandle(self: @This()) *anyopaque {
            return @ptrCast(self.handle);
        }
    };

    try std.testing.expectEqual(raw, Buffer.fromHandle(raw).toHandle());
}

...
`//pjrtx/core` is deprecated bootstrap code.
...
- target descriptions -> `//pjrtx/target`
...
- allocations, lifetimes, streams, events, runtime profile facts ->
  `//pjrtx/runtime/facts`
- stable text summaries and normalization -> `//pjrtx/report`

`docs/specs/package_boundaries_v0.md` is the migration contract. Delete this
package when `rg "//pjrtx/core|pjrtx/core" pjrtx` is empty.
...
- `//pjrtx/target` owns target hardware facts used by the trace report.
- `//pjrtx/compiler/facts` owns source, tensor, graph value, and graph
  instruction facts used by the legacy `TraceReport` shape.
- `//pjrtx/compiler/facts` owns pass, rewrite, fusion, placement, collective,

...
As of 2026-05-19, code under `//pjrtx/...` has initial coverage for:
...
- Phase 6E compiler package boundary hardened: target legality uses a
  compiler-owned V0 capability view and no longer imports `//pjrtx/backend`
- First `//pjrtx/tests/architecture:boundary_test` package guardrail exists:
  compiler/backend/runtime/plugin edges are checked, `//pjrtx/core:sources` is
  forbidden, and the current bootstrap `//pjrtx/core` import baseline is pinned
  so migration can only shrink it.
- First `//pjrtx/target` ownership slice is live: target hardware records,
  dtype rates, structural target validation, and target summary rendering live
  in `//pjrtx/target`; compiler, MLIR state, backend, runtime, and vertical
  execution tests now import target facts directly. `//pjrtx/core` keeps only a
  temporary compatibility bridge for report-shaped legacy callers.
- First `//pjrtx/compiler/facts` ownership slice is live: source IDs, graph
  ...
  checks live under `//pjrtx/compiler/facts`. `//pjrtx/core` temporarily
...
- Compiler and backend implementation code now import those source/graph/tensor
  facts directly from `//pjrtx/compiler/facts`; runtime allocation/profile
  ...
  spellings are gone from live `//pjrtx/...` code outside the temporary
  `core` bridge.
- The next `//pjrtx/compiler/facts` slice is live: MLIR pass records, graph
  ...
  `//pjrtx/core` keeps only compatibility aliases for the legacy `TraceReport`
  container and report writer.
- `//pjrtx/compiler/facts` is now split by ownership into ID, graph, and
...
- First `//pjrtx/compiler/mlir_state` implementation exists: it parses
...
- `//pjrtx/compiler/mlir_state` is now a dedicated Bazel package with local
  `types.zig`, `limits.zig`, `lowering_policy.zig`, and `passes.zig` files. The
  parent `//pjrtx/compiler` target orchestrates compile policy, while the
...
- `//pjrtx/tests/vertical_slice:mlir_state_test` adds a runfiles-backed golden
...
- Backend executable planning now has a backend-owned MLIR bridge target,
  `//pjrtx/backend:mlir_bridge`, which commits concrete backend calls to MLIR
  ...
  compiler dependency on `//pjrtx/backend`.
...
- Runtime allocation planning now has a runtime-owned MLIR bridge target,
  `//pjrtx/runtime:mlir_bridge`, which commits verified buffer reservations,
...
placement, and collective artifacts under `//pjrtx/compiler`, plus a
`//pjrtx/tests/vertical_slice:lowering_tests` target. Fusion, placement, and
...
6. Deepen Zig-owned external MLIR passes for compiler policy where the MLIR C
   ...
   `//pjrtx/compiler/mlir_state:passes`; next, pass callback structs should move
...
- no new package may centralize shared vocabulary as `//pjrtx/core`
...
- `bazel test //pjrtx/...` discovers the skeleton targets
- packages have strict visibility
- `//pjrtx/target` imports no compiler/runtime/backend/plugin/report package
- `//pjrtx/compiler` imports no runtime/backend/plugin package
- no package imports `//pjrtx/core` after migration
- add a short `//pjrtx/...` coding policy note that points to the adapted
...
- the coding policy says schema docs remain centralized under `docs/specs`,
...
- target records and validators live under `//pjrtx/target`
- graph/compiler records live under `//pjrtx/compiler/facts`
- backend records live under `//pjrtx/backend/facts`
- runtime records live under `//pjrtx/runtime/facts`
- deterministic report rendering lives under `//pjrtx/report`
...
- `rg "//pjrtx/core|pjrtx/core" pjrtx` is empty when the migration is complete
...
- `//pjrtx/compiler` owns a typed V0 pass catalog with stable pass names, stages,

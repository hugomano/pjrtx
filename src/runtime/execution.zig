...
const DonationAliasDelta = struct {
...
pub fn executeCompiledExecutable(
    ...
    return executeDevice(executable, allocator, context, device_index, arguments);
}
...
fn tryExecuteBackendExecutable(
    ...
    const completion_event = runtimeEventFromBackendCompletion(context, owned_backend_result.completion);
    defer allocator.free(owned_backend_outputs);

    var wrapped_backend_outputs: usize = 0;
    errdefer {
        for (owned_backend_outputs[wrapped_backend_outputs..]) |output| context.destroyBackendBuffer(output.handle);
    }

    if (owned_backend_outputs.len != executable.outputCount()) return error.Internal;
    for (owned_backend_outputs, 0..) |output, output_index| {
        if (!executable.backendOutputMatches(output_index, output)) return error.Internal;
    }
    ...
    var donation_alias_delta = DonationAliasDelta{};
    errdefer executable.rollbackDonationAlias(donation_alias_delta.output_count, donation_alias_delta.output_bytes);

    for (owned_backend_outputs, 0..) |output, i| {
        const alias = executable.donatedParameterAliasForOutput(i);
        const backend_handle = if (alias) |alias_info| blk: {
            ...
            if (donated_argument.elementType() != output.element_type or !std.mem.eql(i64, donated_argument.dimensions(), output.dims)) return error.Internal;
        ...
        outputs[i] = Buffer.initBackendHandle(
            ...
            if (alias != null) context.destroyBackendBuffer(backend_handle);
...
fn runtimeEventFromBackendCompletion(context: ExecutionContext, completion: backend_api.ExecutionCompletion) Event {
...
fn mapBufferError(err: anyerror) ExecutionError {
    return switch (err) {
...
const ExecutionTestSupport = struct {
...
test "backend output descriptor validation matches executable plan outputs" {
    ...
    var output_shardings = try allocator.alloc(ir.ShardingPlan, 1);
    output_shardings[0] = try ExecutionTestSupport.shardingPlan(allocator, &assignment);
    ...
    try std.testing.expect(compiled.backendOutputMatches(0, .{
        .handle = handle,
        .element_type = .f32,
        .dims = &dims,
        .byte_size = 16,
    ...
    try std.testing.expect(!compiled.backendOutputMatches(1, .{
        .handle = handle,
        .element_type = .f32,
        .dims = &dims,
        .byte_size = 16,
...
test "compiled executable executes through runtime buffers" {
    const client = try ExecutionTestSupport.initMlxMetalClient();
    defer client.deinit();
    const allocator = std.testing.allocator;
    const assignment = [_]i32{0};
    const dims = [_]i64{4};

    var plan = try ExecutionTestSupport.addU8ExecutablePlan(allocator, &assignment, &dims);
    defer plan.deinit();
    ...
    const result = try executeCompiledExecutable(&compiled, allocator, client.executableContext(), 0, &.{ lhs, rhs });
    ...
    try std.testing.expectEqual(@as(usize, 0), outputs[0].hostDebugByteCount());
    ...
    try ExecutionTestSupport.expectBufferBytes(outputs[0], &.{ 11, 22, 33, 44 });
    ...
    try std.testing.expectError(error.InvalidArgument, executeCompiledExecutable(&compiled, allocator, client.executableContext(), 0, &.{ wrong_shard, rhs }));

    Buffer.Testing.setReadiness(lhs, Event.pending());
    try std.testing.expectError(error.BufferNotReady, executeCompiledExecutable(&compiled, allocator, client.executableContext(), 0, &.{ lhs, rhs }));
    lhs.markReady();
    Buffer.Testing.setReadiness(rhs, Event.failed("argument upload failed"));
    try std.testing.expectError(error.BufferReadinessFailed, executeCompiledExecutable(&compiled, allocator, client.executableContext(), 0, &.{ lhs, rhs }));
    Buffer.Testing.setReadiness(rhs, Event.ready());

    Buffer.Testing.setDeviceId(lhs, 1234);
    try std.testing.expectError(error.InvalidArgument, executeCompiledExecutable(&compiled, allocator, client.executableContext(), 0, &.{ lhs, rhs }));
}
...
test "compiled executable transfers donated parameter alias outputs without copying" {
    ...
    parameter_shardings[0] = try ExecutionTestSupport.shardingPlan(allocator, &assignment);
    var output_shardings = try allocator.alloc(ir.ShardingPlan, 1);
    output_shardings[0] = try ExecutionTestSupport.shardingPlan(allocator, &assignment);
    ...
    const result = try executeCompiledExecutable(&compiled, allocator, client.executableContext(), 0, &.{input});
    ...
    try std.testing.expectEqual(before_execute_handle, result.outputs[0].backendHandleForDispatch());
    ...
    try ExecutionTestSupport.expectBufferBytes(result.outputs[0], &data);
...
test "backend pending completion without event handle fails closed" {
    ...
    const event = runtimeEventFromBackendCompletion(client.executableContext(), .{ .kind = .pending });
...
test "compiled executable evicts idle executable cache before output allocation" {
    ...
    var idle_plan = try ExecutionTestSupport.constantU8ExecutablePlan(allocator, &assignment, &.{ 1, 2, 3, 4, 5, 6, 7, 8 });
    ...
    var add_plan = try ExecutionTestSupport.addU8ExecutablePlan(allocator, &assignment, &dims);
    ...
    const result = try executeCompiledExecutable(&add_compiled, allocator, client.executableContext(), 0, &.{ lhs, rhs });
    const outputs = result.outputs;
    defer allocator.free(outputs);
    defer {
        for (outputs) |output| output.deinit();

...
follow-up threads for the new PjRTx architecture under `//pjrtx/...`.
...
This logbook tracks the new architecture described in `docs/` and implemented
under `//pjrtx/...`.

Historical entries are not current instructions. When older entries disagree
with current specs, follow `docs/specs/README.md` and the canonical spec named
...
Consequences:
New architecture code should be rooted under `//pjrtx/...`. Bridges to
`//src/...` are allowed during migration, but new public architecture should
not be mixed into `//src/...`.

Follow-up:
Create Bazel packages under `//pjrtx/...` when implementation starts.

Links:
- `docs/pjrtx_architecture_vision.md`
...
Decision:
Merge backend/kernel-generation content into `docs/pjrtx_architecture_vision.md`
...
Consequences:
`docs/pjrtx_architecture_vision.md` is the canonical architecture document.
`docs/vertical_slice_v0.md` is the first scoped implementation slice.
...
doc. Keep `docs/vertical_slice_v0.md` focused on the V0 workload, artifacts,
...
Follow-up:
Add initial `//pjrtx/tests/vertical_slice` report tests during V0
implementation.
...
Links:
- `docs/pjrtx_architecture_vision.md`
- `docs/specs/compile_pipeline_v0.md`
- `docs/specs/harness_v0.md`
...
Follow-up:
Create `//pjrtx/core` skeleton with trace schema types and report validation.

Links:
- `docs/specs/trace_schema_v0.md`
- `docs/specs/harness_v0.md`
- `docs/specs/target_model_v0.md`
- `docs/specs/compile_pipeline_v0.md`
- `docs/specs/implementation_workplan_v0.md`
...
- `docs/specs/correctness_policy_v0.md`
- `docs/specs/trace_schema_v0.md`
- `docs/specs/harness_v0.md`
- `docs/specs/target_model_v0.md`
- `docs/specs/compile_pipeline_v0.md`
...
- Create `//pjrtx/core` with typed IDs, trace records, target records, report
...
- Keep `README.md` and `//src/...` as bootstrap/legacy source of truth only.
  New architecture work references docs and lands under `//pjrtx/...`.
...
Decision:
Create the first `//pjrtx/...` Bazel packages beside the existing `//src/...`
...
`//src/...` namespace. Future work can grow `//pjrtx/core` toward the trace
schema and target model specs while keeping package boundaries visible.

Verification:
`bazel query //pjrtx/...` discovers the new package tree.
...
Verification:
`bazel test //pjrtx/core:unit_tests --test_output=errors` passes.
`bazel test //pjrtx/... --test_output=errors` passes.
...
and explain records to `//pjrtx/core`. Add `TraceReport` validation for stable
...
Context:
The design specs exist under `docs/`. The implemented `//pjrtx/core` contracts
need nearby intent, but standalone package schema files would duplicate the
central specs and create drift.

Decision:
Keep schema docs centralized under `docs/specs`. Add inline intent comments to
...
a second schema source of truth. Schema changes happen in `docs/specs`; code
...
`pjrtx/tests/vertical_slice/testdata`. Update the report test to construct a
...
Verification:
`bazel test //pjrtx/tests/vertical_slice:report_test --test_output=errors`
passes.
`bazel test //pjrtx/... --test_output=errors` passes.
No `@embedFile`, `@as(`, or `@import("src/...")` remains in `pjrtx/**/*.zig`.

Links:
- `pjrtx/tests/vertical_slice/report_test.zig`
- `pjrtx/tests/vertical_slice/testdata/tiny_trace_report.txt`
- `pjrtx/tests/vertical_slice/BUILD.bazel`
...
Verification:
`bazel test //pjrtx/compiler:unit_tests --test_output=errors` passes.
`bazel test //pjrtx/... --test_output=errors` passes.
No `@embedFile`, `@as(`, or `@import("src/...")` remains in `pjrtx/**/*.zig`.
...
`pjrtx/tests/vertical_slice/testdata/v0_execution_report.txt`. Future changes
to planning, allocation, stream events, or profile metrics must update the
golden intentionally.

Verification:
`bazel test //pjrtx/tests/vertical_slice:execution_test --test_output=errors`
passes.

Links:
- `pjrtx/tests/vertical_slice/execution_test.zig`
- `pjrtx/tests/vertical_slice/testdata/v0_execution_report.txt`
- `pjrtx/tests/vertical_slice/BUILD.bazel`
...
Consequences:
The new `//pjrtx/...` tree now uses Metal and NPU naming consistently.
...
- `pjrtx/backend/backend.zig`
- `pjrtx/compiler/compiler.zig`
- `pjrtx/tests/vertical_slice/metal_bridge_test.zig`
- `docs/specs/target_model_v0.md`
...
`BackendKernelGraphEdge` to `//pjrtx/backend`. V0 creates one Metal MLS graph
...
Verification:
`bazel test //pjrtx/backend:unit_tests //pjrtx/tests/vertical_slice:metal_bridge_test --test_output=errors`
passes.
...
Verification:
`bazel test //pjrtx/tests/vertical_slice:metal_bridge_test --test_output=errors`
passes.
...
Decision:
Add `BackendExecutablePlan` and `BackendExecutableCall` to `//pjrtx/backend`.
...
Verification:
`bazel test //pjrtx/runtime:unit_tests --test_output=errors` passes.
...
`//pjrtx/compiler`. Input setup now parses program format, target kind, and
...
Decision:
Add a new `//pjrtx/compiler:mlir_c_api` C/C++ boundary, register the func,
...
Verification:
`bazel test //pjrtx/compiler:unit_tests //pjrtx/tests/vertical_slice:import_tests //pjrtx/tests/vertical_slice:report_test --test_output=errors`
passes.
`bazel test //pjrtx/... --test_output=errors` passes.
No `@embedFile`, `@as(`, or `@import("src/...")` remains in `pjrtx/**/*.zig`.
...
- `pjrtx/tests/vertical_slice/import_test.zig`
...
Verification:
`bazel test //pjrtx/compiler:unit_tests --test_output=errors` passes.
`bazel test //pjrtx/tests/vertical_slice:import_tests --test_output=errors`
passes.
...
Decision:
Add backend capability sets under `//pjrtx/backend` with feature and dtype
...
Verification:
`bazel test //pjrtx/backend:unit_tests //pjrtx/compiler:unit_tests --test_output=errors`
passes.
...
Verification:
`bazel test //pjrtx/runtime:unit_tests //pjrtx/tests/vertical_slice:execution_test --test_output=errors`
passes.
...
Verification:
`bazel test //pjrtx/compiler:unit_tests //pjrtx/tests/vertical_slice:lowering_tests --test_output=errors`
passes.

Links:
- `pjrtx/compiler/compiler.zig`
- `pjrtx/tests/vertical_slice/lowering_test.zig`
...
- `pjrtx/core/core.zig`
- `pjrtx/compiler/compiler.zig`
- `pjrtx/tests/fixtures/identity_broadcast.mlir`
- `pjrtx/tests/vertical_slice/lowering_test.zig`
- `docs/specs/trace_schema_v0.md`
...
- `pjrtx/compiler/compiler.zig`
- `pjrtx/backend/backend.zig`
- `pjrtx/tests/fixtures/identity_reshape_transpose.mlir`
- `pjrtx/tests/fixtures/non_identity_transpose.mlir`
- `pjrtx/tests/vertical_slice/lowering_test.zig`
...
Decision:
Add a typed V0 compiler pass catalog in `//pjrtx/compiler`. Each catalog entry
...
- `pjrtx/compiler/compiler.zig`
- `pjrtx/tests/vertical_slice/lowering_test.zig`
- `docs/specs/compiler_pass_pipeline_v0.md`
...
Context:
The vision and harness boundary rules say `//pjrtx/compiler` must not depend on
`//pjrtx/backend`, but the V0 compiler was importing backend capability helpers
...
Decision:
Remove the `//pjrtx/backend` dependency from `//pjrtx/compiler`. The compiler
...
Consequences:
The Bazel compiler target now depends on `//pjrtx/core` and MLIR/StableHLO/
Shardy C API targets, not `//pjrtx/backend`. This preserves the intended
layering while keeping V0 capability data explicit.

Verification:
`bazel test //pjrtx/compiler:unit_tests //pjrtx/tests/vertical_slice:lowering_tests //pjrtx/tests/vertical_slice:backend_binding_test --test_output=errors`
passes.
...
Verification:
`bazel test //pjrtx/runtime:unit_tests //pjrtx/backend:unit_tests //pjrtx/tests/vertical_slice:metal_bridge_test //pjrtx/tests/vertical_slice:execution_test --test_output=errors`
passes.
...
Verification:
`bazel test //pjrtx/core:unit_tests //pjrtx/tests/vertical_slice:report_test --test_output=errors`
passes.
...
Verification:
`bazel test //pjrtx/core:unit_tests //pjrtx/tests/vertical_slice:report_test //pjrtx/tests/vertical_slice:lowering_tests //pjrtx/tests/vertical_slice:execution_test --test_output=errors`
passes.
...
Verification:
`bazel test //pjrtx/backend:unit_tests //pjrtx/tests/vertical_slice:metal_bridge_test //pjrtx/tests/vertical_slice:execution_test --test_output=errors`
passes.
...
Verification:
`bazel test //pjrtx/backend:unit_tests //pjrtx/runtime:unit_tests //pjrtx/tests/vertical_slice:metal_bridge_test //pjrtx/tests/vertical_slice:execution_test --test_output=errors`
passes.
...
Verification:
`bazel test //pjrtx/core:unit_tests //pjrtx/compiler:unit_tests //pjrtx/backend:unit_tests //pjrtx/runtime:unit_tests //pjrtx/tests/vertical_slice:report_test //pjrtx/tests/vertical_slice:lowering_tests //pjrtx/tests/vertical_slice:metal_bridge_test //pjrtx/tests/vertical_slice:execution_test --test_output=errors`
passes.
...
Decision:
Add `docs/specs/compiler_pass_pipeline_v0.md`. The spec defines pass families,
...
Verification:
`bazel test //pjrtx/compiler:unit_tests //pjrtx/tests/vertical_slice:lowering_tests //pjrtx/tests/vertical_slice:import_tests //pjrtx/tests/vertical_slice:execution_test --test_output=errors`
passes.
...
Decision:
Add `docs/specs/xla_coverage_map_v0.md` and link it from the architecture
...
Links:
- `docs/specs/xla_coverage_map_v0.md`
- `docs/pjrtx_architecture_vision.md`
- `docs/specs/compiler_pass_pipeline_v0.md`
...
Links:
- `pjrtx/compiler/compiler.zig`
- `pjrtx/tests/fixtures/large_tiling.mlir`
- `pjrtx/tests/vertical_slice/lowering_test.zig`
- `docs/specs/trace_schema_v0.md`
...
Verification:
`bazel test //pjrtx/compiler:unit_tests //pjrtx/core:unit_tests //pjrtx/tests/vertical_slice:report_test //pjrtx/tests/vertical_slice:lowering_tests --test_output=errors`
passes.
...
Verification:
`bazel test //pjrtx/core:unit_tests //pjrtx/compiler:unit_tests //pjrtx/tests/vertical_slice:lowering_tests --test_output=errors`
passes.
...
- `pjrtx/core/core.zig`
- `pjrtx/compiler/compiler.zig`
- `pjrtx/tests/fixtures/all_reduce_channel.mlir`
- `pjrtx/tests/vertical_slice/lowering_test.zig`
- `docs/specs/compiler_pass_pipeline_v0.md`
- `docs/specs/trace_schema_v0.md`
...
Links:
- `docs/specs/mlir_state_machine_compiler_v0.md`
- `docs/specs/README.md`
- `docs/pjrtx_architecture_vision.md`
- `docs/specs/compiler_pass_pipeline_v0.md`
- `docs/specs/trace_schema_v0.md`
...
Consequences:
The next implementation step should be `//pjrtx/compiler/mlir_state` or an
...
Links:
- `docs/specs/pjrtx_mlir_dialect_v0.md`
...
Decision:
Add `//pjrtx/compiler/mlir_state`. It owns a small `MlirSession`, parses
...
Verification:
`bazel test //pjrtx/compiler/mlir_state:unit_tests --test_output=errors` passes.
...
Verification:
`bazel test //pjrtx/compiler:unit_tests //pjrtx/compiler/mlir_state:unit_tests //pjrtx/tests/vertical_slice:lowering_tests --test_output=errors` passes.
...
Verification:
`bazel test //pjrtx/compiler/mlir_state:unit_tests //pjrtx/compiler:unit_tests --test_output=errors` passes.
...
Verification:
`bazel test //pjrtx/compiler/mlir_state:unit_tests //pjrtx/tests/vertical_slice:mlir_state_test //pjrtx/compiler:unit_tests --test_output=errors` passes.

Links:
- `pjrtx/compiler/mlir_state/package.zig`
- `pjrtx/tests/vertical_slice/mlir_state_test.zig`
- `docs/specs/pjrtx_mlir_dialect_v0.md`
...
Links:
- `pjrtx/compiler/mlir_state/package.zig`
- `pjrtx/tests/vertical_slice/testdata/mlir_state_after_fusion.txt`
- `docs/specs/pjrtx_mlir_dialect_v0.md`
...
Decision:
Add `//pjrtx/tests/vertical_slice:mlir_state_test` and
...
Verification:
`bazel test //pjrtx/tests/vertical_slice:mlir_state_test --test_output=errors`
passes.
...
`//pjrtx/dialects/...` and `//pjrtx/compiler/mlir/...` only for narrow dialect
...
still only derived from the extracted report by `//pjrtx/backend`.

Decision:
Add `backend_executable_planned` to the MLIR module state machine and introduce
`//pjrtx/backend:mlir_bridge`. Backend planning still owns the policy for
...
was still only derived inside `//pjrtx/backend` immediately before summary
printing.
...
still only derived inside `//pjrtx/runtime` before stream planning and profile
summaries.

Decision:
Add `runtime_allocation_planned` to the MLIR module state machine and introduce
`//pjrtx/runtime:mlir_bridge`. Runtime planning still owns capacity checks,
...
`//pjrtx/compiler/mlir_state`. Runtime join extraction accepts
...
Verification:
`bazel test //pjrtx/compiler/mlir_state:unit_tests //pjrtx/tests/vertical_slice:execution_test //pjrtx/tests/vertical_slice:metal_bridge_test --test_output=errors` passes.
...
Verification:
`bazel test //pjrtx/compiler/mlir_state:unit_tests //pjrtx/tests/vertical_slice:execution_test --test_output=errors` passes.
...
`//pjrtx/compiler/mlir_state`. Backend executable extraction accepts
...
`//pjrtx/compiler/mlir_state`. Allocation extraction accepts
...
Decision:
Add `extractRuntimeProfileEvents` to `//pjrtx/compiler/mlir_state`. Extraction
...
`//pjrtx/runtime` and report summaries.
...
Verification:
`bazel test //pjrtx/compiler/mlir_state:unit_tests //pjrtx/compiler:unit_tests //pjrtx/tests/vertical_slice:execution_test --test_output=errors` passes.
...
Verification:
`bazel test //pjrtx/compiler/mlir_state:unit_tests //pjrtx/compiler:unit_tests //pjrtx/tests/vertical_slice:execution_test //pjrtx/tests/vertical_slice:lowering_tests --test_output=errors` passes.
...
Verification:
`bazel test //pjrtx/compiler/mlir_state:unit_tests //pjrtx/compiler:unit_tests //pjrtx/tests/architecture:boundary_test --test_output=errors` passes.
...
Verification:
`bazel test //pjrtx/compiler/mlir_state:unit_tests //pjrtx/compiler:unit_tests //pjrtx/tests/architecture:boundary_test --test_output=errors` passes. Full `bazel test //pjrtx/... --test_output=errors` also passes.
...
Context:
`//pjrtx/compiler/mlir_state` had become a large implementation target inside
...
`//pjrtx/compiler/mlir_state` Bazel package. Split public state-machine and
...
Reasoning:
The package tree should describe ownership. `//pjrtx/compiler` orchestrates
compile policy; `//pjrtx/compiler/mlir_state` owns MLIR state transitions and
...
deps now point at `//pjrtx/compiler/mlir_state`. The parent compiler package no
...
Decision:
Add a canonical ownership table to `docs/specs/README.md` and reduce repeated
...
`bazel test //pjrtx/... --test_output=errors` should still be run after this
docs-only pass.

Links:
- `docs/specs/README.md`
- `docs/specs/package_boundaries_v0.md`
...
Context:
The next correction is structural: `//pjrtx/core` became a central vocabulary
...
on new `//pjrtx/core` imports. The dialect implementation should bootstrap
`//pjrtx/dialects` and `//pjrtx/mlir` before replacing individual temporary

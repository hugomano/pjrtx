...
expected_core_lines() {
  case "$1" in
    ...
    pjrtx/tests/vertical_slice/BUILD.bazel) echo 5 ;;
    pjrtx/tests/vertical_slice/backend_binding_test.zig) echo 1 ;;
    pjrtx/tests/vertical_slice/execution_test.zig) echo 1 ;;
    pjrtx/tests/vertical_slice/import_test.zig) echo 1 ;;
    pjrtx/tests/vertical_slice/lowering_test.zig) echo 1 ;;
    pjrtx/tests/vertical_slice/report_test.zig) echo 1 ;;
    *) echo 0 ;;
  esac
}
...
check_core_baseline() {
  ...
  while IFS= read -r file; do
    actual="$(grep -Ec 'pjrtx/core|//pjrtx/core' "$file" || true)"
    expected="$(expected_core_lines "$file")"
    if [[ "${actual}" != "${expected}" ]]; then
      fail "unexpected //pjrtx/core reference count in ${file}: expected ${expected}, got ${actual}"
      grep -En 'pjrtx/core|//pjrtx/core' "$file" >&2 || true
    fi
  done < <(source_files)
}
...
forbid_pattern \
  ...
  "pjrtx/compiler/" \
  'pjrtx/(backend|runtime|plugin)|//pjrtx/(backend|runtime|plugin)'

forbid_pattern \
  "compiler facts must not depend on deprecated core" \
  "pjrtx/compiler/facts/" \
  'pjrtx/core|//pjrtx/core'

forbid_pattern \
  "target must not depend on compiler, backend, runtime, plugin, or core packages" \
  "pjrtx/target/" \
  'pjrtx/(compiler|backend|runtime|plugin|core)|//pjrtx/(compiler|backend|runtime|plugin|core)'

forbid_pattern \
  "backend must not depend on runtime or plugin packages" \
  "pjrtx/backend/" \
  'pjrtx/(runtime|plugin)|//pjrtx/(runtime|plugin)'

forbid_pattern \
  "runtime must not depend on backend or plugin packages" \
  "pjrtx/runtime/" \
  'pjrtx/(backend|plugin)|//pjrtx/(backend|plugin)'

forbid_pattern \
  "plugin must not depend on backend implementation packages" \
  "pjrtx/plugin/" \
  'pjrtx/backend|//pjrtx/backend'

forbid_pattern \
  "deprecated core package must not depend on the migrated pjrtx packages" \
  "pjrtx/core/" \
  'pjrtx/(compiler|backend|runtime|plugin)|//pjrtx/(compiler|backend|runtime|plugin)'

core_sources_matches="$(source_files | xargs grep -En '//pjrtx/core:sources' 2>/dev/null || true)"
if [[ -n "${core_sources_matches}" ]]; then
  fail "do not reintroduce //pjrtx/core:sources"

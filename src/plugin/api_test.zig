...
const backend_option = plugin.Options.backend;
const default_memory_kind = plugin.MemoryKinds.device;
const Executable = executable_mod.Executable;
const LoadedExecutableHandle = handles.LoadedExecutable(Executable);
const EventCallbackRecordHandle = handles.UserArg(EventCallbackRecord);
const PjrtEvent = events_mod.Event;
const PjrtError = errors_mod.Error;
...
const EventCallbackRecord = struct {
    count: usize = 0,
    ready_count: usize = 0,
    error_count: usize = 0,
...
test "event callbacks bridge PJRT errors and pending runtime transitions" {
    ...
    const pending_runtime_event = plugin.allocator().create(runtime.Event) catch return error.OutOfMemory;
...
test "loaded executable execute chains u8 StableHLO ops through PJRT argument lists" {
    ...
    compile_args.program = &program;
    try expectOk(api.PJRT_Client_Compile.?(&compile_args));
    Executable.Testing.setDonatedParameters(LoadedExecutableHandle.ref(compile_args.executable), try plugin.allocator().dupe(u32, &.{0}));
    defer {
        var destroy_args = std.mem.zeroes(c.PJRT_LoadedExecutable_Destroy_Args);
        destroy_args.struct_size = c.PJRT_LoadedExecutable_Destroy_Args_STRUCT_SIZE;

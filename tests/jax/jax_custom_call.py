...
def assert_backend_custom_call_trace(trace_text: str) -> None:
    ...
    if not compile_traces:
        raise AssertionError("custom-call run did not emit a compile trace")
    if not execute_traces:
        raise AssertionError("custom-call run did not emit an execute trace")
    for trace in compile_traces:
        if trace_int(trace, "backend_executable") != 1 or trace_int(trace, "unlowered") != 0:
            raise AssertionError(f"custom-call compile left the MLX backend path: {trace}")
    for trace in execute_traces:
        if trace_int(trace, "backend_candidate") != 1:
            raise AssertionError(f"custom-call execute did not use backend buffers: {trace}")
        if trace_int(trace, "backend_completion_pending") != 0:
            raise AssertionError(f"custom-call execute left a pending MLX completion: {trace}")

...
def assert_fast_path_trace(name: str, trace_text: str, *, expect_compile: bool) -> None:
    ...
    if expect_compile and not compile_traces:
        raise AssertionError(f"{name}: trace did not include compile events")
    if not execute_traces:
        raise AssertionError(f"{name}: trace did not include execute events")
    for trace in compile_traces:
        if trace_int(trace, "backend_executable") != 1 or trace_int(trace, "unlowered") != 0:
            raise AssertionError(f"{name}: compile did not stay on backend executable path: {trace}")
    for trace in compile_cache_traces:
        if trace_int(trace, "cache_pressure_failed") != 0:
            raise AssertionError(f"{name}: compile cache pressure was not satisfied: {trace}")
    for trace in execute_traces:
        if trace_int(trace, "backend_candidate") != 1:
            raise AssertionError(f"{name}: execute used a non-backend argument/output path: {trace}")
        if trace_int(trace, "cache_pressure_failed") != 0:
            raise AssertionError(f"{name}: execute cache pressure was not satisfied: {trace}")
        if trace_int(trace, "backend_completion_pending") != 0:
            raise AssertionError(f"{name}: MLX execute reported a pending backend completion: {trace}")

...
`next/docs/architecture_logbook.md` so the reasoning does not get lost in chat
history.
...
The new architecture lives in `next/docs/` and should be implemented under
`//next/pjrtx/...`. It is allowed to bridge to `//src/...` during migration, but the
...
- `next/docs/vertical_slice_v0.md`: the first vertical slice scope.
- `next/docs/specs/mlir_state_machine_compiler_v0.md`: the intended MLIR-first
  compiler state machine where compiler-middle facts live in MLIR dialect
  state and Zig records are extracted views.
- `next/docs/specs/pjrtx_mlir_dialect_v0.md`: the first concrete PjRTx MLIR
...
- `next/docs/specs/final_mlir_dialect_op_pass_architecture_v0.md`: the destination
  PjRTx dialect/op/pass architecture after the current attribute-state bridge.
- `next/docs/specs/package_boundaries_v0.md`: the segregated package graph and the
  no-`//next/pjrtx/core` ownership rule.
- `next/docs/specs/trace_schema_v0.md`: stable IDs, provenance records, profile
  events, and report joins extracted from compiler/runtime state.
- `next/docs/specs/target_model_v0.md`: target, memory, bandwidth, execution-unit,
  dtype-rate, and fingerprint records.
- `next/docs/specs/compile_pipeline_v0.md`: no-fallback compile gates from input to
  executable creation.
- `next/docs/specs/compiler_pass_pipeline_v0.md`: explainable XLA-like compiler pass
  families, pass contracts, ordering, and invariants.
- `next/docs/specs/xla_coverage_map_v0.md`: local XLA coverage map and the design
...
- `next/docs/specs/harness_v0.md`: fast tests, golden reports, debug dumps, and
  hardware execution policy.
- `next/docs/specs/correctness_policy_v0.md`: strict math defaults and reference
  oracle policy.
- `next/docs/specs/implementation_workplan_v0.md`: concrete packages, build phases,
...
must not grow around a central `//next/pjrtx/core` package. Types live with the
...
Backend and codegen packages for the new architecture live under `//next/pjrtx/...`.
Use `next/docs/specs/package_boundaries_v0.md` for the canonical package graph.
...
`next/docs/specs/package_boundaries_v0.md`. The canonical final dialect/op/pass
destination lives in
`next/docs/specs/final_mlir_dialect_op_pass_architecture_v0.md`.

New architecture code should live under Bazel packages rooted at `//next/pjrtx/...`.
...
for new `//next/pjrtx/...` code. The relevant principle is not cosmetic formatting;
...
- zero hidden debt in core architecture: do not merge temporary execution paths,
  implicit fallback, unbounded queues, or opaque ownership into `//next/pjrtx/...`
...
- `//next/pjrtx/compiler` may depend on `//next/pjrtx/target`,
  `//next/pjrtx/compiler/facts`, `//next/pjrtx/mlir`, `//next/pjrtx/dialects`, and
  MLIR/StableHLO/Shardy C API targets
- `//next/pjrtx/compiler` may depend on narrow `//next/pjrtx/compiler/mlir` C-compatible
  shim targets for MLIR extension points that are not exposed by the public C
  API
- real PjRTx dialect definitions live under `//next/pjrtx/dialects/...` and use
  TableGen/C++ only for MLIR-native dialect plumbing
- `//next/pjrtx/compiler` must not depend on `//next/pjrtx/runtime`
- `//next/pjrtx/compiler` must not depend on `//next/pjrtx/backend`
- `//next/pjrtx/runtime` must not import Metal C symbols
- no new package may depend on `//next/pjrtx/core`
- `//next/pjrtx/plugin` must stay a PJRT adapter
...
//next/pjrtx/vertical_slice:execution_test
//next/pjrtx/architecture:boundary_test
//next/pjrtx/performance:smoke_test
```
...
next/pjrtx/fixtures/elementwise_chain.mlir
next/pjrtx/fixtures/unsupported_convolution.mlir
next/pjrtx/fixtures/bad_dot_shape.mlir
...
- `//next/pjrtx/compiler` cannot import runtime/backend/plugin
- `//next/pjrtx/target` cannot import compiler/runtime/backend/plugin/report
- no package can import `//next/pjrtx/core` after the migration is complete
- `//next/pjrtx/runtime` cannot mention Metal C symbols
- `//next/pjrtx/plugin` cannot mention Metal backend shim symbols
- `//next/pjrtx/backend` implementations cannot leak into target/compiler/runtime
  fact packages

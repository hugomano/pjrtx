...
int main() {
  ...
  if (copied > 0) {
    ...
    PjrtxMlxMetalBuffer *buffer = pjrtx_mlx_metal_buffer_from_host(
        devices[0].ordinal, input, sizeof(input));
    ...
    assert(pjrtx_mlx_metal_buffer_copy_to_host(difference, difference_output,
                                               sizeof(difference_output)) == 1);
    for (int i = 0; i < static_cast<int>(sizeof(input)); ++i) {
    ...
    assert(pjrtx_mlx_metal_buffer_copy_to_host(bitwise_and, bitwise_and_output,
                                               sizeof(bitwise_and_output)) ==
           1);
    for (int i = 0; i < static_cast<int>(sizeof(input)); ++i) {
      assert(bitwise_and_output[i] ==
             static_cast<unsigned char>(input[i] & rhs[i]));
    }
    pjrtx_mlx_metal_buffer_destroy(bitwise_and);

    PjrtxMlxMetalBuffer *popcnt =
        pjrtx_mlx_metal_buffer_unary_u8(buffer, PJRTX_MLX_METAL_UNARY_POPCNT);
    assert(popcnt != nullptr);
    unsigned char popcnt_output[sizeof(input)] = {};
    assert(pjrtx_mlx_metal_buffer_copy_to_host(popcnt, popcnt_output,
                                               sizeof(popcnt_output)) == 1);
    ...
    PjrtxMlxMetalBuffer *clz =
        pjrtx_mlx_metal_buffer_unary_u8(buffer, PJRTX_MLX_METAL_UNARY_CLZ);
    assert(clz != nullptr);
    ...
    assert(pjrtx_mlx_metal_buffer_copy_to_host(quotient, quotient_output,
                                               sizeof(quotient_output)) == 1);
    for (int i = 0; i < static_cast<int>(sizeof(input)); ++i) {
    ...
    assert(pjrtx_mlx_metal_buffer_copy_to_host(negated, negated_output,
                                               sizeof(negated_output)) == 1);
    for (int i = 0; i < static_cast<int>(sizeof(input)); ++i) {
...

    {
      ...
      PjrtxMlxMetalBuffer *matrix = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, matrix_input, sizeof(matrix_input),
          PJRTX_MLX_METAL_DTYPE_F32, broadcast_dims, 2);
      PjrtxMlxMetalBuffer *scalar = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, scalar_input, sizeof(scalar_input),
          PJRTX_MLX_METAL_DTYPE_F32, scalar_dims, 0);
      assert(matrix != nullptr);
      assert(scalar != nullptr);
      PjrtxMlxMetalBuffer *broadcast_sum = pjrtx_mlx_metal_buffer_binary_out(
          matrix, scalar, PJRTX_MLX_METAL_U8_BINARY_ADD, broadcast_dims, 2);
      ...
      for (int i = 0; i < 6; ++i) {
        assert(
            near(broadcast_sum_output[i], matrix_input[i] + scalar_input[0]));
      }
      PjrtxMlxMetalBuffer *broadcast_compare = pjrtx_mlx_metal_buffer_compare(
...

    {
      ...
      PjrtxMlxMetalBuffer *abs_complex = pjrtx_mlx_metal_buffer_unary(
          complex_buffer, PJRTX_MLX_METAL_UNARY_ABS);
      assert(abs_complex != nullptr);
      assert(pjrtx_mlx_metal_buffer_size(abs_complex) == 2 * sizeof(float));
      float abs_complex_output[2] = {};
      assert(
          pjrtx_mlx_metal_buffer_copy_to_host(abs_complex, abs_complex_output,
    ...
    assert(iota != nullptr);
    float iota_output[6] = {};
    assert(pjrtx_mlx_metal_buffer_copy_to_host(iota, iota_output,
                                               sizeof(iota_output)) == 1);
    {
    ...
    PjrtxMlxMetalBuffer *partition_id = pjrtx_mlx_metal_buffer_partition_id(
        devices[0].ordinal, PJRTX_MLX_METAL_DTYPE_U32, 7);
    assert(partition_id != nullptr);
    uint32_t partition_output = 0;
    assert(pjrtx_mlx_metal_buffer_copy_to_host(partition_id, &partition_output,
                                               sizeof(partition_output)) == 1);
...

    {
      ...
      assert(pjrtx_mlx_metal_async_h2d_write(transfer, 0, input,
                                             2 * sizeof(float)) == 1);
      assert(pjrtx_mlx_metal_async_h2d_write(transfer, 2 * sizeof(float),
                                             input + 2,
                                             2 * sizeof(float)) == 1);
      ...
      float output[4] = {};
      assert(pjrtx_mlx_metal_buffer_copy_to_host(async_buffer, output,
                                                 sizeof(output)) == 1);
      for (int i = 0; i < 4; ++i) {
...

    {
      ...
      PjrtxMlxMetalBuffer *q_buffer = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, q_input, sizeof(q_input),
          PJRTX_MLX_METAL_DTYPE_F32, q_dims, 3);
      PjrtxMlxMetalBuffer *k_buffer = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, k_input, sizeof(k_input),
          PJRTX_MLX_METAL_DTYPE_F32, kv_dims, 3);
      PjrtxMlxMetalBuffer *v_buffer = pjrtx_mlx_metal_buffer_from_host_typed(
...

    {
      ...
      const float v_input[] = {10.0f, 0.0f,  20.0f, 0.0f,
                               0.0f,  30.0f, 0.0f,  40.0f};
      const uint32_t token_index = 0;
      PjrtxMlxMetalBuffer *q_buffer = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, q_input, sizeof(q_input),
          PJRTX_MLX_METAL_DTYPE_F32, q_dims, 3);
      ...
      assert(pjrtx_mlx_metal_buffer_copy_to_host(
                 attention, attention_output, sizeof(attention_output)) == 1);
      const float expected[] = {10.0f, 0.0f, 10.0f, 0.0f,
    ...
    PjrtxMlxMetalBuffer *f32_buffer = pjrtx_mlx_metal_buffer_from_host_typed(
    ...
    assert(pjrtx_mlx_metal_buffer_copy_to_host(complex_buffer, complex_output,
                                               sizeof(complex_output)) == 1);
    for (int i = 0; i < 3; ++i) {
    ...
    float real_output[3] = {};
    float imag_output[3] = {};
    assert(pjrtx_mlx_metal_buffer_copy_to_host(real_part, real_output,
                                               sizeof(real_output)) == 1);
    assert(pjrtx_mlx_metal_buffer_copy_to_host(imag_part, imag_output,
                                               sizeof(imag_output)) == 1);
    for (int i = 0; i < 3; ++i) {
    ...
    assert(
        pjrtx_mlx_metal_buffer_copy_to_host(f32_zero_like, f32_zero_like_output,
    ...
    PjrtxMlxMetalBuffer *clamp_min = pjrtx_mlx_metal_buffer_from_host_typed(
        devices[0].ordinal, &clamp_min_scalar, sizeof(clamp_min_scalar),
        PJRTX_MLX_METAL_DTYPE_F32, nullptr, 0);
    PjrtxMlxMetalBuffer *clamp_max = pjrtx_mlx_metal_buffer_from_host_typed(
    ...
    assert(pjrtx_mlx_metal_buffer_copy_to_host(clamped, clamp_output,
                                               sizeof(clamp_output)) == 1);
    {
    ...
    assert(pjrtx_mlx_metal_buffer_copy_to_host(window_sum, window_sum_output,
                                               sizeof(window_sum_output)) == 1);
    {
    ...
    assert(pjrtx_mlx_metal_buffer_copy_to_host(window_max, window_max_output,
                                               sizeof(window_max_output)) == 1);
    ...
    PjrtxMlxMetalBuffer *f16_window_sum = pjrtx_mlx_metal_buffer_reduce_window(
        f16_window_buffer, PJRTX_MLX_METAL_REDUCE_SUM, reduce_window_rank1,
        reduce_window_stride1, reduce_window_base1, reduce_window_dilation1,
        reduce_window_no_pad, reduce_window_no_pad, 1, f16_window_output_dims,
        1);
    PjrtxMlxMetalBuffer *f16_window_max = pjrtx_mlx_metal_buffer_reduce_window(
        f16_window_buffer, PJRTX_MLX_METAL_REDUCE_MAX, reduce_window_rank1,
        reduce_window_stride1, reduce_window_base1, reduce_window_dilation1,
        reduce_window_no_pad, reduce_window_no_pad, 1, f16_window_output_dims,
        1);
    assert(f16_window_sum != nullptr);
    assert(f16_window_max != nullptr);
    PjrtxMlxMetalBuffer *f16_window_sum_f32 = pjrtx_mlx_metal_buffer_astype(
        f16_window_sum, PJRTX_MLX_METAL_DTYPE_F32);
    PjrtxMlxMetalBuffer *f16_window_max_f32 = pjrtx_mlx_metal_buffer_astype(
        f16_window_max, PJRTX_MLX_METAL_DTYPE_F32);
    ...
    PjrtxMlxMetalBuffer *pool_values = pjrtx_mlx_metal_buffer_from_host_typed(
        devices[0].ordinal, pool_values_input, sizeof(pool_values_input),
        PJRTX_MLX_METAL_DTYPE_F32, pool_dims, 2);
    PjrtxMlxMetalBuffer *pool_indices = pjrtx_mlx_metal_buffer_from_host_typed(
    ...
    assert(
        pjrtx_mlx_metal_buffer_copy_to_host(pooled_values, pooled_values_output,
    ...
    PjrtxMlxMetalBuffer *u32_bits = pjrtx_mlx_metal_buffer_from_host_typed(
        devices[0].ordinal, u32_bits_input, sizeof(u32_bits_input),
        PJRTX_MLX_METAL_DTYPE_U32, u32_bits_dims, 1);
    assert(u32_bits != nullptr);
    PjrtxMlxMetalBuffer *u32_bits_as_f32 = pjrtx_mlx_metal_buffer_view_dtype(
    ...
    PjrtxMlxMetalBuffer *f16_buffer = pjrtx_mlx_metal_buffer_from_host_typed(
    ...
    PjrtxMlxMetalBuffer *f16_as_f32 =
        pjrtx_mlx_metal_buffer_astype(f16_buffer, PJRTX_MLX_METAL_DTYPE_F32);
    ...
    assert(pjrtx_mlx_metal_buffer_copy_to_host(f16_as_f32, f16_as_f32_output,
                                               sizeof(f16_as_f32_output)) == 1);
    ...
    PjrtxMlxMetalBuffer *bf16_buffer = pjrtx_mlx_metal_buffer_from_host_typed(
...

    {
      ...
      PjrtxMlxMetalBuffer *lhs = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, lhs_f16, sizeof(lhs_f16),
          PJRTX_MLX_METAL_DTYPE_F16, mat_dims, 2);
      PjrtxMlxMetalBuffer *rhs = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, rhs_f16, sizeof(rhs_f16),
          PJRTX_MLX_METAL_DTYPE_F16, mat_dims, 2);
      assert(lhs != nullptr);
      assert(rhs != nullptr);
      PjrtxMlxMetalBuffer *product = pjrtx_mlx_metal_buffer_dot_general(
          lhs, rhs, nullptr, 0, nullptr, 0, lhs_contract, 1, rhs_contract, 1,
          mat_dims, 2);
      assert(product != nullptr);
      ...
      assert(pjrtx_mlx_metal_buffer_copy_to_host(product_f32, product_output,
                                                 sizeof(product_output)) == 1);
      ...
      PjrtxMlxMetalBuffer *lhs_b = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, lhs_bf16, sizeof(lhs_bf16),
          PJRTX_MLX_METAL_DTYPE_BF16, mat_dims, 2);
      PjrtxMlxMetalBuffer *rhs_b = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, rhs_bf16, sizeof(rhs_bf16),
          PJRTX_MLX_METAL_DTYPE_BF16, mat_dims, 2);
      assert(lhs_b != nullptr);
      assert(rhs_b != nullptr);
      PjrtxMlxMetalBuffer *product_b = pjrtx_mlx_metal_buffer_dot_general(
          lhs_b, rhs_b, nullptr, 0, nullptr, 0, lhs_contract, 1, rhs_contract,
          1, mat_dims, 2);
      assert(product_b != nullptr);
      assert(pjrtx_mlx_metal_buffer_size(product_b) == sizeof(lhs_bf16));
      PjrtxMlxMetalBuffer *product_b_f32 =
          pjrtx_mlx_metal_buffer_astype(product_b, PJRTX_MLX_METAL_DTYPE_F32);
      assert(product_b_f32 != nullptr);
      float product_b_output[4] = {};
      assert(
          pjrtx_mlx_metal_buffer_copy_to_host(product_b_f32, product_b_output,
    ...
    {
      const int64_t lhs_dims[] = {2, 3};
      const int64_t rhs_dims[] = {4, 3};
      const int64_t out_dims[] = {2, 4};
      const int64_t lhs_contract[] = {1};
      const int64_t rhs_contract[] = {1};
      const float lhs_values[] = {1.0f, 2.0f, 3.0f, -1.0f, 0.5f, 4.0f};
      const float rhs_values[] = {
          1.0f, 0.0f,  2.0f, -1.0f, 3.0f, 0.5f,
          2.0f, -2.0f, 1.0f, 0.25f, 0.5f, -1.0f,
      };
      PjrtxMlxMetalBuffer *lhs = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, lhs_values, sizeof(lhs_values),
          PJRTX_MLX_METAL_DTYPE_F32, lhs_dims, 2);
      PjrtxMlxMetalBuffer *rhs = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, rhs_values, sizeof(rhs_values),
          PJRTX_MLX_METAL_DTYPE_F32, rhs_dims, 2);
      assert(lhs != nullptr);
      assert(rhs != nullptr);
      PjrtxMlxMetalBuffer *product = pjrtx_mlx_metal_buffer_dot_general(
          lhs, rhs, nullptr, 0, nullptr, 0, lhs_contract, 1, rhs_contract, 1,
      ...
      for (int m = 0; m < 2; ++m) {
        for (int n = 0; n < 4; ++n) {
          float expected = 0.0f;
          for (int k = 0; k < 3; ++k) {
            expected += lhs_values[m * 3 + k] * rhs_values[n * 3 + k];
          }
          assert(near_relaxed(product_output[m * 4 + n], expected));
        }
      }
      pjrtx_mlx_metal_buffer_destroy(product);
      pjrtx_mlx_metal_buffer_destroy(rhs);
    ...
    {
      const int64_t lhs_dims[] = {2, 2, 3};
      const int64_t rhs_dims[] = {2, 3, 4};
      const int64_t out_dims[] = {2, 2, 4};
      const int64_t batch[] = {0};
      const int64_t lhs_contract[] = {2};
      const int64_t rhs_contract[] = {1};
      float lhs_values[2 * 2 * 3] = {};
      float rhs_values[2 * 3 * 4] = {};
      for (int i = 0; i < 2 * 2 * 3; ++i) {
        lhs_values[i] = static_cast<float>((i % 7) - 3);
      }
      for (int i = 0; i < 2 * 3 * 4; ++i) {
        rhs_values[i] = static_cast<float>((i % 11) - 5) / 3.0f;
      }
      PjrtxMlxMetalBuffer *lhs = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, lhs_values, sizeof(lhs_values),
          PJRTX_MLX_METAL_DTYPE_F32, lhs_dims, 3);
      PjrtxMlxMetalBuffer *rhs = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, rhs_values, sizeof(rhs_values),
          PJRTX_MLX_METAL_DTYPE_F32, rhs_dims, 3);
      assert(lhs != nullptr);
      assert(rhs != nullptr);
      PjrtxMlxMetalBuffer *product = pjrtx_mlx_metal_buffer_dot_general(
          lhs, rhs, batch, 1, batch, 1, lhs_contract, 1, rhs_contract, 1,
          out_dims, 3);
      assert(product != nullptr);
      float product_output[2 * 2 * 4] = {};
      assert(pjrtx_mlx_metal_buffer_copy_to_host(product, product_output,
                                                 sizeof(product_output)) == 1);
      for (int b = 0; b < 2; ++b) {
        for (int m = 0; m < 2; ++m) {
          for (int n = 0; n < 4; ++n) {
            float expected = 0.0f;
            for (int k = 0; k < 3; ++k) {
              const int lhs_index = ((b * 2 + m) * 3 + k);
              const int rhs_index = ((b * 3 + k) * 4 + n);
              expected += lhs_values[lhs_index] * rhs_values[rhs_index];
            }
            const int out_index = ((b * 2 + m) * 4 + n);
            assert(near_relaxed(product_output[out_index], expected));
          }
        }
      }
      pjrtx_mlx_metal_buffer_destroy(product);
    ...
    {
      const int64_t lhs_dims[] = {2, 2, 3};
      const int64_t rhs_dims[] = {3, 4};
      const int64_t out_dims[] = {2, 2, 4};
      const int64_t lhs_contract[] = {2};
      const int64_t rhs_contract[] = {0};
      float lhs_values[2 * 2 * 3] = {};
      float rhs_values[3 * 4] = {};
      for (int i = 0; i < 2 * 2 * 3; ++i) {
        lhs_values[i] = static_cast<float>((i % 9) - 4) / 2.0f;
      }
      for (int i = 0; i < 3 * 4; ++i) {
        rhs_values[i] = static_cast<float>((i % 5) - 2);
      }
      PjrtxMlxMetalBuffer *lhs = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, lhs_values, sizeof(lhs_values),
          PJRTX_MLX_METAL_DTYPE_F32, lhs_dims, 3);
...

    {
      ...
      assert(q_buffer != nullptr);
      assert(k_buffer != nullptr);
      PjrtxMlxMetalBuffer *attn_scores = pjrtx_mlx_metal_buffer_dot_general(
      ...
      assert(pjrtx_mlx_metal_buffer_copy_to_host(attn_scores, attn_output,
                                                 sizeof(attn_output)) == 1);
      for (int b = 0; b < 2; ++b) {
...

    {
      ...
      PjrtxMlxMetalBuffer *prob_buffer = pjrtx_mlx_metal_buffer_from_host_typed(
      ...
      assert(v_buffer != nullptr);
      PjrtxMlxMetalBuffer *context = pjrtx_mlx_metal_buffer_dot_general(
          prob_buffer, v_buffer, lhs_batch, 1, rhs_batch, 1, lhs_contract, 1,
          rhs_contract, 1, out_dims, 4);
      assert(context != nullptr);
      float context_output[2 * 3 * 4 * 5] = {};
      assert(pjrtx_mlx_metal_buffer_copy_to_host(context, context_output,
                                                 sizeof(context_output)) == 1);
      for (int b = 0; b < 2; ++b) {
        for (int hq = 0; hq < 3; ++hq) {
          for (int q = 0; q < 4; ++q) {
            for (int d = 0; d < 5; ++d) {
...

    {
      ...
      assert(pjrtx_mlx_metal_buffer_size(f16_reduce_sum) ==
             sizeof(uint16_t) * 2);
      PjrtxMlxMetalBuffer *f16_reduce_sum_f32 = pjrtx_mlx_metal_buffer_astype(
          f16_reduce_sum, PJRTX_MLX_METAL_DTYPE_F32);
      PjrtxMlxMetalBuffer *f16_reduce_max_f32 = pjrtx_mlx_metal_buffer_astype(
          f16_reduce_max, PJRTX_MLX_METAL_DTYPE_F32);
      PjrtxMlxMetalBuffer *f16_reduce_min_f32 = pjrtx_mlx_metal_buffer_astype(
          f16_reduce_min, PJRTX_MLX_METAL_DTYPE_F32);
      ...
      assert(pjrtx_mlx_metal_buffer_size(bf16_reduce_sum) ==
             sizeof(uint16_t) * 2);
      PjrtxMlxMetalBuffer *bf16_reduce_sum_f32 = pjrtx_mlx_metal_buffer_astype(
          bf16_reduce_sum, PJRTX_MLX_METAL_DTYPE_F32);
...

    {
      ...
      assert(pjrtx_mlx_metal_buffer_copy_to_host(argmax_out_values, out_values,
                                                 sizeof(out_values)) == 1);
      assert(pjrtx_mlx_metal_buffer_copy_to_host(
...

    {
      ...
      PjrtxMlxMetalBuffer *state_a = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, seed_state, sizeof(seed_state),
          PJRTX_MLX_METAL_DTYPE_U64, state_dims, 1);
      PjrtxMlxMetalBuffer *state_b = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, seed_state, sizeof(seed_state),
          PJRTX_MLX_METAL_DTYPE_U64, state_dims, 1);
      ...
      uint32_t random_a[8] = {};
      uint32_t random_b[8] = {};
      assert(pjrtx_mlx_metal_buffer_copy_to_host(next_state_a, new_state_a,
                                                 sizeof(new_state_a)) == 1);
      assert(pjrtx_mlx_metal_buffer_copy_to_host(next_state_b, new_state_b,
                                                 sizeof(new_state_b)) == 1);
      assert(pjrtx_mlx_metal_buffer_copy_to_host(bits_a, random_a,
                                                 sizeof(random_a)) == 1);
      assert(pjrtx_mlx_metal_buffer_copy_to_host(bits_b, random_b,
                                                 sizeof(random_b)) == 1);
      ...
      const uint32_t expected_random[] = {0x1324f11aU, 0xf5ee7bf6U, 0xac0a940eU,
                                          0x9145f7a4U, 0x2ae1effeU, 0x5af1ca39U,
                                          0x2594e3dfU, 0x326d7bf1U};
      for (int i = 0; i < 8; ++i) {
...

    {
      ...
      const uint64_t seed_state[] = {0x123456789abcdef0ULL,
                                     0x0fedcba987654321ULL};
      PjrtxMlxMetalBuffer *state = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, seed_state, sizeof(seed_state),
          PJRTX_MLX_METAL_DTYPE_U64, state_dims, 1);
      ...
      assert(pjrtx_mlx_metal_buffer_rng_bit_generator(
                 state, PJRTX_MLX_METAL_DTYPE_U64, random_dims, 2, &next_state,
                 &bits) == 1);
      assert(next_state != nullptr);
      assert(bits != nullptr);
      uint64_t new_state[2] = {};
      uint64_t random[8] = {};
      assert(pjrtx_mlx_metal_buffer_copy_to_host(next_state, new_state,
                                                 sizeof(new_state)) == 1);
      assert(pjrtx_mlx_metal_buffer_copy_to_host(bits, random,
                                                 sizeof(random)) == 1);
      assert(new_state[0] == seed_state[0]);
      assert(new_state[1] == seed_state[1] + 8);
      const uint64_t expected_random[] = {
          0x2ae1effe1324f11aULL, 0x5af1ca39f5ee7bf6ULL, 0x2594e3dfac0a940eULL,
          0x326d7bf19145f7a4ULL, 0x03866c09756254aeULL, 0x8b8081ab0c7ae7aaULL,
    ...
    assert(f32_rhs_buffer != nullptr);
    {
      ...
      float rng_output[8] = {};
      assert(pjrtx_mlx_metal_buffer_copy_to_host(rng_normal, rng_output,
                                                 sizeof(rng_output)) == 1);
      for (int i = 0; i < 8; ++i) {
    ...
    if (f32_sum != nullptr) {
      ...
      assert(pjrtx_mlx_metal_buffer_copy_to_host(f32_sum, f32_sum_output,
                                                 sizeof(f32_sum_output)) == 1);
      for (int i = 0; i < 3; ++i) {
        assert(near(f32_sum_output[i], f32_input[i] + f32_rhs[i]));
      }
      pjrtx_mlx_metal_buffer_destroy(f32_sum);
    }
    assert(setenv("PJRTX_MSL_EXECUTE", "1", 1) == 0);
    PjrtxMlxMetalBuffer *f32_msl_sum = pjrtx_mlx_metal_buffer_binary(
        f32_buffer, f32_rhs_buffer, PJRTX_MLX_METAL_U8_BINARY_ADD);
    assert(f32_msl_sum != nullptr);
    float f32_msl_sum_output[3] = {};
    assert(pjrtx_mlx_metal_buffer_copy_to_host(f32_msl_sum, f32_msl_sum_output,
                                               sizeof(f32_msl_sum_output)) ==
           1);
    for (int i = 0; i < 3; ++i) {
      assert(near(f32_msl_sum_output[i], f32_input[i] + f32_rhs[i]));
    }
    pjrtx_mlx_metal_buffer_destroy(f32_msl_sum);
    assert(unsetenv("PJRTX_MSL_EXECUTE") == 0);
    PjrtxMlxMetalBuffer *f32_metalcpp_sum =
        pjrtx_metalcpp_buffer_dense_binary_out(f32_buffer, f32_rhs_buffer,
                                               PJRTX_MLX_METAL_U8_BINARY_ADD,
                                               f32_dims, 1);
    assert(f32_metalcpp_sum != nullptr);
    float f32_metalcpp_sum_output[3] = {};
    assert(pjrtx_mlx_metal_buffer_copy_to_host(
               f32_metalcpp_sum, f32_metalcpp_sum_output,
               sizeof(f32_metalcpp_sum_output)) == 1);
    for (int i = 0; i < 3; ++i) {
      assert(near(f32_metalcpp_sum_output[i], f32_input[i] + f32_rhs[i]));
    }
    pjrtx_mlx_metal_buffer_destroy(f32_metalcpp_sum);
    {
      const char fusion_source[] =
          "#include <metal_stdlib>\n"
          "using namespace metal;\n"
          "kernel void pjrtx_test_dense_fusion("
          ...
          "  out0[elem] = (in0[elem] + in1[elem]) * in1[elem];"
          "}\n";
      const PjrtxMetalCppTensorSpec fusion_inputs[] = {
          {PJRTX_MLX_METAL_DTYPE_F32, f32_dims, 1},
          {PJRTX_MLX_METAL_DTYPE_F32, f32_dims, 1},
      };
      const PjrtxMetalCppTensorSpec fusion_outputs[] = {
          {PJRTX_MLX_METAL_DTYPE_F32, f32_dims, 1},
      };
      const PjrtxMetalCppFusionKernelCreateArgs fusion_args = {
          sizeof(PjrtxMetalCppFusionKernelCreateArgs),
          devices[0].ordinal,
          "pjrtx_test_dense_fusion",
          sizeof("pjrtx_test_dense_fusion") - 1,
          fusion_source,
          sizeof(fusion_source) - 1,
          fusion_inputs,
          2,
          fusion_outputs,
          1,
      ...
      PjrtxMetalCppFusionKernel *fusion_kernel =
          pjrtx_metalcpp_fusion_kernel_create(&fusion_args);
      assert(fusion_kernel != nullptr);
      PjrtxMlxMetalBuffer *fusion_inputs_raw[] = {f32_buffer, f32_rhs_buffer};
      for (int iteration = 0; iteration < 2; ++iteration) {
        PjrtxMlxMetalBuffer **fusion_outputs_raw = nullptr;
        uint64_t fusion_output_count = 0;
        assert(pjrtx_metalcpp_fusion_kernel_execute(
                   fusion_kernel, fusion_inputs_raw, 2, &fusion_outputs_raw,
                   &fusion_output_count) == 1);
        assert(fusion_outputs_raw != nullptr);
        assert(fusion_output_count == 1);
        float fusion_output[3] = {};
        assert(pjrtx_mlx_metal_buffer_copy_to_host(fusion_outputs_raw[0],
                                                   fusion_output,
                                                   sizeof(fusion_output)) == 1);
        for (int i = 0; i < 3; ++i) {
          assert(
              near(fusion_output[i], (f32_input[i] + f32_rhs[i]) * f32_rhs[i]));
        }
        pjrtx_mlx_metal_buffer_destroy(fusion_outputs_raw[0]);
        pjrtx_metalcpp_fusion_output_array_destroy(fusion_outputs_raw);
      }
      pjrtx_metalcpp_fusion_kernel_destroy(fusion_kernel);
    }
    {
      const char executable_program_source[] =
          "#include <metal_stdlib>\n"
          "using namespace metal;\n"
          "kernel void pjrtx_test_executable_program("
          "device const float* in0 [[buffer(0)]], "
          "device const float* in1 [[buffer(1)]], "
          "device float* out0 [[buffer(2)]], "
          "constant uint& count [[buffer(3)]], "
          "uint elem [[thread_position_in_grid]]) {"
          "  if (elem >= count) return;"
          "  out0[elem] = (in0[elem] * in1[elem]) + in0[elem];"
          "}\n";
      const PjrtxMetalCppTensorSpec executable_program_inputs[] = {
          {PJRTX_MLX_METAL_DTYPE_F32, f32_dims, 1},
          {PJRTX_MLX_METAL_DTYPE_F32, f32_dims, 1},
      };
      const PjrtxMetalCppTensorSpec executable_program_outputs[] = {
          {PJRTX_MLX_METAL_DTYPE_F32, f32_dims, 1},
      };
      const PjrtxMetalCppExecutableProgramCreateArgs executable_program_args = {
          sizeof(PjrtxMetalCppExecutableProgramCreateArgs),
          devices[0].ordinal,
          "pjrtx_test_executable_program",
          sizeof("pjrtx_test_executable_program") - 1,
          executable_program_source,
          sizeof(executable_program_source) - 1,
          executable_program_inputs,
          2,
          executable_program_outputs,
          1,
          3,
          128,
      };
      PjrtxMetalCppExecutableProgram *executable_program =
          pjrtx_metalcpp_executable_program_create(&executable_program_args);
      assert(executable_program != nullptr);
      PjrtxMlxMetalBuffer *executable_program_inputs_raw[] = {f32_buffer,
                                                              f32_rhs_buffer};
      PjrtxMlxMetalBuffer **executable_program_outputs_raw = nullptr;
      uint64_t executable_program_output_count = 0;
      assert(pjrtx_metalcpp_executable_program_execute(
                 executable_program, executable_program_inputs_raw, 2,
                 &executable_program_outputs_raw,
                 &executable_program_output_count) == 1);
      assert(executable_program_outputs_raw != nullptr);
      assert(executable_program_output_count == 1);
      float executable_program_output[3] = {};
      assert(pjrtx_mlx_metal_buffer_copy_to_host(
                 executable_program_outputs_raw[0], executable_program_output,
                 sizeof(executable_program_output)) == 1);
      for (int i = 0; i < 3; ++i) {
        assert(near(executable_program_output[i],
                    (f32_input[i] * f32_rhs[i]) + f32_input[i]));
      }
      pjrtx_mlx_metal_buffer_destroy(executable_program_outputs_raw[0]);
      pjrtx_metalcpp_executable_program_output_array_destroy(
          executable_program_outputs_raw);
      pjrtx_metalcpp_executable_program_destroy(executable_program);
    }
    {
      const char add_source[] = "#include <metal_stdlib>\n"
                                "using namespace metal;\n"
                                "kernel void pjrtx_test_executable_add("
                                "device const float* in0 [[buffer(0)]], "
                                "device const float* in1 [[buffer(1)]], "
                                "device float* out0 [[buffer(2)]], "
                                "constant uint& count [[buffer(3)]], "
                                "uint elem [[thread_position_in_grid]]) {"
                                "  if (elem >= count) return;"
                                "  out0[elem] = in0[elem] + in1[elem];"
                                "}\n";
      const char multiply_source[] =
          "#include <metal_stdlib>\n"
          "using namespace metal;\n"
          "kernel void pjrtx_test_executable_multiply("
          "device const float* in0 [[buffer(0)]], "
          "device const float* in1 [[buffer(1)]], "
          "device float* out0 [[buffer(2)]], "
          "constant uint& count [[buffer(3)]], "
      ...
      const uint64_t program_inputs[] = {0, 1};
      const uint64_t program_outputs[] = {3};
      const uint64_t add_inputs[] = {0, 1};
      const uint64_t add_outputs[] = {2};
      const uint64_t multiply_inputs[] = {2, 1};
      const uint64_t multiply_outputs[] = {3};
      const PjrtxMetalCppExecutableStep steps[] = {
          {
              "pjrtx_test_executable_add",
              sizeof("pjrtx_test_executable_add") - 1,
              add_source,
              sizeof(add_source) - 1,
              add_inputs,
              2,
          ...
          {
              "pjrtx_test_executable_multiply",
              sizeof("pjrtx_test_executable_multiply") - 1,
              multiply_source,
              sizeof(multiply_source) - 1,
              multiply_inputs,
              2,
              multiply_outputs,
              1,
      ...
      PjrtxMetalCppExecutableProgram *program =
          pjrtx_metalcpp_executable_program_create(&args);
      assert(program != nullptr);
      PjrtxMlxMetalBuffer *inputs_raw[] = {f32_buffer, f32_rhs_buffer};
      PjrtxMlxMetalBuffer **outputs_raw = nullptr;
      uint64_t output_count = 0;
      assert(pjrtx_metalcpp_executable_program_execute(
                 program, inputs_raw, 2, &outputs_raw, &output_count) == 1);
      assert(outputs_raw != nullptr);
      assert(output_count == 1);
      float output[3] = {};
      assert(pjrtx_mlx_metal_buffer_copy_to_host(outputs_raw[0], output,
                                                 sizeof(output)) == 1);
      for (int i = 0; i < 3; ++i) {
        assert(near(output[i], (f32_input[i] + f32_rhs[i]) * f32_rhs[i]));
      }
      pjrtx_mlx_metal_buffer_destroy(outputs_raw[0]);
      pjrtx_metalcpp_executable_program_output_array_destroy(outputs_raw);
      pjrtx_metalcpp_executable_program_destroy(program);
    }
    ...
    pjrtx_mlx_metal_buffer_destroy(f32_custom_sum);
    {
      ...
      PjrtxMlxMetalBuffer *while_state = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, while_state_input, sizeof(while_state_input),
          PJRTX_MLX_METAL_DTYPE_F32, while_dims, 1);
      PjrtxMlxMetalBuffer *while_limit = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, while_limit_input, sizeof(while_limit_input),
          PJRTX_MLX_METAL_DTYPE_F32, while_dims, 1);
      PjrtxMlxMetalBuffer *while_step = pjrtx_mlx_metal_buffer_from_host_typed(
      ...
      assert(pjrtx_mlx_metal_buffer_copy_to_host(while_output, while_result,
                                                 sizeof(while_result)) == 1);
      ...
      PjrtxMlxMetalBuffer *scalar_state =
          pjrtx_mlx_metal_buffer_from_host_typed(
              devices[0].ordinal, &scalar_state_input,
              sizeof(scalar_state_input), PJRTX_MLX_METAL_DTYPE_F32, nullptr,
              0);
      PjrtxMlxMetalBuffer *scalar_limit =
          pjrtx_mlx_metal_buffer_from_host_typed(
              devices[0].ordinal, &scalar_limit_input,
              sizeof(scalar_limit_input), PJRTX_MLX_METAL_DTYPE_F32, nullptr,
              0);
      PjrtxMlxMetalBuffer *scalar_step = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, &while_step_input, sizeof(while_step_input),
          PJRTX_MLX_METAL_DTYPE_F32, nullptr, 0);
      ...
      PjrtxMlxMetalBuffer *scalar_output =
          pjrtx_mlx_metal_buffer_while_f32_lt_add(scalar_state, scalar_limit,
                                                  scalar_step, nullptr, 0, 8);
      assert(scalar_output != nullptr);
      float scalar_result = 0.0f;
      assert(pjrtx_mlx_metal_buffer_copy_to_host(scalar_output, &scalar_result,
                                                 sizeof(scalar_result)) == 1);
      ...
      PjrtxMlxMetalBuffer *down_state = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, &down_state_input, sizeof(down_state_input),
          PJRTX_MLX_METAL_DTYPE_F32, nullptr, 0);
      PjrtxMlxMetalBuffer *down_limit = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, &down_limit_input, sizeof(down_limit_input),
          PJRTX_MLX_METAL_DTYPE_F32, nullptr, 0);
      PjrtxMlxMetalBuffer *down_step = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, &while_step_input, sizeof(while_step_input),
          PJRTX_MLX_METAL_DTYPE_F32, nullptr, 0);
      ...
      assert(pjrtx_mlx_metal_buffer_copy_to_host(down_output, &down_result,
                                                 sizeof(down_result)) == 1);
      ...
      PjrtxMlxMetalBuffer *bf16_state = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, &bf16_zero, sizeof(bf16_zero),
          PJRTX_MLX_METAL_DTYPE_BF16, nullptr, 0);
      PjrtxMlxMetalBuffer *bf16_limit = pjrtx_mlx_metal_buffer_from_host_typed(
          devices[0].ordinal, &bf16_one, sizeof(bf16_one),
          PJRTX_MLX_METAL_DTYPE_BF16, nullptr, 0);
      PjrtxMlxMetalBuffer *bf16_step = pjrtx_mlx_metal_buffer_from_host_typed(
      ...
      assert(pjrtx_mlx_metal_buffer_copy_to_host(bf16_output, &bf16_result,
                                                 sizeof(bf16_result)) == 1);
    ...
    assert(pjrtx_mlx_metal_buffer_copy_to_host(f32_atan2, f32_atan2_output,
                                               sizeof(f32_atan2_output)) == 1);
    for (int i = 0; i < 3; ++i) {
      assert(near(f32_atan2_output[i], std::atan2(f32_input[i], f32_rhs[i])));
    }
    pjrtx_mlx_metal_buffer_destroy(f32_atan2);
    PjrtxMlxMetalBuffer *f32_sqrt =
        pjrtx_mlx_metal_buffer_unary(f32_buffer, PJRTX_MLX_METAL_UNARY_SQRT);
    if (f32_sqrt != nullptr) {
      float f32_sqrt_output[3] = {};
      assert(pjrtx_mlx_metal_buffer_copy_to_host(f32_sqrt, f32_sqrt_output,
                                                 sizeof(f32_sqrt_output)) == 1);
    ...
    PjrtxMlxMetalBuffer *f32_expm1 =
        pjrtx_mlx_metal_buffer_unary(f32_buffer, PJRTX_MLX_METAL_UNARY_EXPM1);
    assert(f32_expm1 != nullptr);
    float f32_expm1_output[3] = {};
    assert(pjrtx_mlx_metal_buffer_copy_to_host(f32_expm1, f32_expm1_output,
                                               sizeof(f32_expm1_output)) == 1);
    for (int i = 0; i < 3; ++i) {
      assert(near_relaxed(f32_expm1_output[i], std::expm1(f32_input[i])));
    }
    pjrtx_mlx_metal_buffer_destroy(f32_expm1);
    PjrtxMlxMetalBuffer *f32_cbrt =
        pjrtx_mlx_metal_buffer_unary(f32_buffer, PJRTX_MLX_METAL_UNARY_CBRT);
    assert(f32_cbrt != nullptr);
    float f32_cbrt_output[3] = {};
    assert(pjrtx_mlx_metal_buffer_copy_to_host(f32_cbrt, f32_cbrt_output,
                                               sizeof(f32_cbrt_output)) == 1);
    for (int i = 0; i < 3; ++i) {
    ...
    assert(
        pjrtx_mlx_metal_buffer_copy_to_host(f32_round_afz, f32_round_afz_output,
    ...
    assert(
        pjrtx_mlx_metal_buffer_copy_to_host(f32_isfinite, f32_isfinite_output,
    ...
    PjrtxMlxMetalBuffer *transposed =
        pjrtx_mlx_metal_buffer_transpose(transpose_buffer, permutation, 2);
    if (transposed != nullptr) {
      float transposed_output[6] = {};
      assert(pjrtx_mlx_metal_buffer_copy_to_host(transposed, transposed_output,
                                                 sizeof(transposed_output)) ==
             1);
    ...
    PjrtxMlxMetalBuffer *broadcasted = pjrtx_mlx_metal_buffer_broadcast_in_dim(
    ...
    const float slice_input[] = {1.0f, 2.0f, 3.0f, 4.0f,  5.0f,  6.0f,
                                 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f};
    PjrtxMlxMetalBuffer *slice_buffer = pjrtx_mlx_metal_buffer_from_host_typed(
        devices[0].ordinal, slice_input, sizeof(slice_input),
        PJRTX_MLX_METAL_DTYPE_F32, slice_dims, 2);
    assert(slice_buffer != nullptr);
    PjrtxMlxMetalBuffer *sliced =
        pjrtx_mlx_metal_buffer_slice(slice_buffer, slice_start, slice_limit,
                                     slice_strides, 2, slice_output_dims, 2);
    if (sliced != nullptr) {
      float slice_output[4] = {};
      assert(pjrtx_mlx_metal_buffer_copy_to_host(sliced, slice_output,
                                                 sizeof(slice_output)) == 1);
    ...
    PjrtxMlxMetalBuffer *dynamic_sliced = pjrtx_mlx_metal_buffer_dynamic_slice(
    ...
    {
      const float expected[] = {1.0f,   2.0f, 3.0f, 4.0f,   5.0f,   100.0f,
                                101.0f, 8.0f, 9.0f, 102.0f, 103.0f, 12.0f};
      for (int i = 0; i < 12; ++i) {
    ...
    PjrtxMlxMetalBuffer *pad_buffer = pjrtx_mlx_metal_buffer_from_host_typed(
    ...
    const float reverse_input[] = {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f};
    ...
    assert(pjrtx_mlx_metal_buffer_copy_to_host(reversed, reverse_output,
                                               sizeof(reverse_output)) == 1);
    {
    ...
    const float gather_operand_input[] = {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f};
    ...
    PjrtxMlxMetalBuffer *gather_zero = pjrtx_mlx_metal_buffer_from_host_typed(
        devices[0].ordinal, gather_zero_input, sizeof(gather_zero_input),
        PJRTX_MLX_METAL_DTYPE_S32, gather_indices_dims, 1);
    PjrtxMlxMetalBuffer *gather_bound = pjrtx_mlx_metal_buffer_from_host_typed(
    ...
    PjrtxMlxMetalBuffer *gather_normalized_indices =
        pjrtx_mlx_metal_buffer_select(gather_negative_mask, gather_bound,
                                      gather_indices, gather_indices_dims, 1);
    ...
    assert(pjrtx_mlx_metal_buffer_copy_to_host(gathered, gather_output,
                                               sizeof(gather_output)) == 1);
    {
    ...
    assert(
        pjrtx_mlx_metal_buffer_copy_to_host(gathered_axis1, gather_axis1_output,
    ...
    assert(pjrtx_mlx_metal_buffer_copy_to_host(gathered_nd, gather_nd_output,
                                               sizeof(gather_nd_output)) == 1);
    {
    ...
    PjrtxMlxMetalBuffer *batched_gathered = pjrtx_mlx_metal_buffer_gather(
        ...
        batched_gather_operand_batching, 1, batched_gather_start_batching, 1, 2,
        batched_gather_slice_sizes, 3, batched_gather_offset_dims, 1,
    ...
    assert(pjrtx_mlx_metal_buffer_copy_to_host(scattered, scatter_output,
                                               sizeof(scatter_output)) == 1);
    {
    ...
    assert(
        pjrtx_mlx_metal_buffer_copy_to_host(scatter_added, scatter_add_output,
    ...
    PjrtxMlxMetalBuffer *point_scattered = pjrtx_mlx_metal_buffer_scatter(
        ...
        nullptr, 0, 1, PJRTX_MLX_METAL_SCATTER_SET, point_scatter_dims, 2);
    ...
    PjrtxMlxMetalBuffer *point_scatter_added = pjrtx_mlx_metal_buffer_scatter(
        point_scatter_operand, point_scatter_indices, point_scatter_updates,
        point_scatter_axes, 2, point_scatter_axes, 2, nullptr, 0, nullptr, 0,
        nullptr, 0, 1, PJRTX_MLX_METAL_SCATTER_ADD, point_scatter_dims, 2);
    ...
    PjrtxMlxMetalBuffer *window_scattered = pjrtx_mlx_metal_buffer_scatter(
        ...
        PJRTX_MLX_METAL_SCATTER_SET, window_scatter_dims, 2);
    ...
    PjrtxMlxMetalBuffer *window_scatter_added = pjrtx_mlx_metal_buffer_scatter(
        ...
        window_scatter_axis, 1, window_scatter_axis, 1,
        window_scatter_update_window, 1, nullptr, 0, nullptr, 0, 1,
        PJRTX_MLX_METAL_SCATTER_ADD, window_scatter_dims, 2);
    ...
    PjrtxMlxMetalBuffer *batched_scattered = pjrtx_mlx_metal_buffer_scatter(
        ...
        batched_scatter_updates, batched_scatter_axis, 1, batched_scatter_axis,
        1, batched_scatter_update_window, 1, batched_scatter_input_batching, 1,
        batched_scatter_indices_batching, 1, 2, PJRTX_MLX_METAL_SCATTER_SET,
        batched_scatter_dims, 3);
    ...
    {
      const float expected[] = {
          104.0f, 105.0f, 106.0f, 107.0f, 4.0f,   5.0f,   6.0f,   7.0f,
          100.0f, 101.0f, 102.0f, 103.0f, 12.0f,  13.0f,  14.0f,  15.0f,
          108.0f, 109.0f, 110.0f, 111.0f, 112.0f, 113.0f, 114.0f, 115.0f,
      };
      for (int i = 0; i < 24; ++i) {
    ...
    PjrtxMlxMetalBuffer *batched_scatter_added = pjrtx_mlx_metal_buffer_scatter(
        ...
        batched_scatter_updates, batched_scatter_axis, 1, batched_scatter_axis,
        1, batched_scatter_update_window, 1, batched_scatter_input_batching, 1,
        batched_scatter_indices_batching, 1, 2, PJRTX_MLX_METAL_SCATTER_ADD,
        batched_scatter_dims, 3);
    ...
    {
      const float expected[] = {
          104.0f, 106.0f, 108.0f, 110.0f, 4.0f,   5.0f,   6.0f,   7.0f,
          108.0f, 110.0f, 112.0f, 114.0f, 12.0f,  13.0f,  14.0f,  15.0f,
          124.0f, 126.0f, 128.0f, 130.0f, 132.0f, 134.0f, 136.0f, 138.0f,
      };
      for (int i = 0; i < 24; ++i) {
    ...
    float sort_output[6] = {};
    assert(pjrtx_mlx_metal_buffer_copy_to_host(sorted, sort_output,
                                               sizeof(sort_output)) == 1);
    {
    ...
    assert(pjrtx_mlx_metal_buffer_copy_to_host(order, order_output,
                                               sizeof(order_output)) == 1);
    {
    ...
    PjrtxMlxMetalBuffer *sorted_values = pjrtx_mlx_metal_buffer_take_along_axis(
        value_buffer, order, 1, sort_dims, 2);
    assert(sorted_values != nullptr);
    int32_t value_output[6] = {};
    assert(pjrtx_mlx_metal_buffer_copy_to_host(sorted_values, value_output,
                                               sizeof(value_output)) == 1);
    {
    ...
    assert(pjrtx_mlx_metal_buffer_copy_to_host(pred_reduce_all, pred_all_output,
                                               sizeof(pred_all_output)) == 1);
    assert(pjrtx_mlx_metal_buffer_copy_to_host(pred_reduce_any, pred_any_output,
                                               sizeof(pred_any_output)) == 1);
    ...
    const float concat_rhs_input[] = {5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 10.0f};
    PjrtxMlxMetalBuffer *concat_lhs = pjrtx_mlx_metal_buffer_from_host_typed(
    ...
    PjrtxMlxMetalBuffer *concat_rhs = pjrtx_mlx_metal_buffer_from_host_typed(
    ...
    if (concatenated != nullptr) {
      ...
      assert(pjrtx_mlx_metal_buffer_copy_to_host(concatenated, concat_output,
                                                 sizeof(concat_output)) == 1);
    ...
    PjrtxMlxMetalBuffer *conv_lhs = pjrtx_mlx_metal_buffer_from_host_typed(
        devices[0].ordinal, conv_input, sizeof(conv_input),
        PJRTX_MLX_METAL_DTYPE_F32, conv_input_dims, 3);
    assert(conv_lhs != nullptr);
    PjrtxMlxMetalBuffer *conv_rhs = pjrtx_mlx_metal_buffer_from_host_typed(
    ...
    const float conv2d_input[] = {1.0f, 2.0f, 3.0f, 4.0f, 5.0f,
                                  6.0f, 7.0f, 8.0f, 9.0f};
    const float conv2d_kernel[] = {1.0f, 1.0f, 1.0f, 1.0f};
    PjrtxMlxMetalBuffer *conv2d_lhs = pjrtx_mlx_metal_buffer_from_host_typed(
        devices[0].ordinal, conv2d_input, sizeof(conv2d_input),
        PJRTX_MLX_METAL_DTYPE_F32, conv2d_input_dims, 4);
    assert(conv2d_lhs != nullptr);
    PjrtxMlxMetalBuffer *conv2d_rhs = pjrtx_mlx_metal_buffer_from_host_typed(
    ...
    assert(pjrtx_mlx_metal_buffer_copy_to_host(convolved2d, conv2d_output,
                                               sizeof(conv2d_output)) == 1);
    ...
    PjrtxMlxMetalBuffer *grouped_convolved = pjrtx_mlx_metal_buffer_convolution(
        grouped_conv_lhs, grouped_conv_rhs, conv2d_stride, conv2d_pad,
        conv2d_pad, conv2d_dilation, conv2d_dilation, conv2d_reversal, 2, 2,
        grouped_conv_output_dims, 4);
    ...
    const float conv3d_kernel[] = {1.0f, 1.0f, 1.0f, 1.0f,
                                   1.0f, 1.0f, 1.0f, 1.0f};
    PjrtxMlxMetalBuffer *conv3d_lhs = pjrtx_mlx_metal_buffer_from_host_typed(
        devices[0].ordinal, conv3d_input, sizeof(conv3d_input),
        PJRTX_MLX_METAL_DTYPE_F32, conv3d_input_dims, 5);
    assert(conv3d_lhs != nullptr);
    PjrtxMlxMetalBuffer *conv3d_rhs = pjrtx_mlx_metal_buffer_from_host_typed(
    ...
    assert(pjrtx_mlx_metal_buffer_copy_to_host(convolved3d, conv3d_output,
                                               sizeof(conv3d_output)) == 1);
    ...
    PjrtxMlxMetalBuffer *rfft_buffer = pjrtx_mlx_metal_buffer_from_host_typed(
        devices[0].ordinal, rfft_input, sizeof(rfft_input),
        PJRTX_MLX_METAL_DTYPE_F32, rfft_dims, 1);
    assert(rfft_buffer != nullptr);
    PjrtxMlxMetalBuffer *rfft =
        pjrtx_mlx_metal_buffer_fft(rfft_buffer, PJRTX_MLX_METAL_RFFT,
                                   rfft_lengths, 1, rfft_output_dims, 1);
    assert(rfft != nullptr);
    float rfft_output[6] = {};
    assert(pjrtx_mlx_metal_buffer_copy_to_host(rfft, rfft_output,
                                               sizeof(rfft_output)) == 1);
    ...
    PjrtxMlxMetalBuffer *chol_buffer = pjrtx_mlx_metal_buffer_from_host_typed(
    ...
    assert(chol != nullptr);
    float chol_output[4] = {};
    assert(pjrtx_mlx_metal_buffer_copy_to_host(chol, chol_output,
                                               sizeof(chol_output)) == 1);
    ...
    PjrtxMlxMetalBuffer *tri_a_buffer = pjrtx_mlx_metal_buffer_from_host_typed(
        devices[0].ordinal, tri_a, sizeof(tri_a), PJRTX_MLX_METAL_DTYPE_F32,
        chol_dims, 2);
    assert(tri_a_buffer != nullptr);
    PjrtxMlxMetalBuffer *tri_b_buffer = pjrtx_mlx_metal_buffer_from_host_typed(
        devices[0].ordinal, tri_b, sizeof(tri_b), PJRTX_MLX_METAL_DTYPE_F32,
        chol_dims, 2);
    ...
    assert(tri != nullptr);
    float tri_output[4] = {};
    assert(pjrtx_mlx_metal_buffer_copy_to_host(tri, tri_output,
                                               sizeof(tri_output)) == 1);
  ...
  assert(pjrtx_mlx_metal_buffer_astype(nullptr, PJRTX_MLX_METAL_DTYPE_F32) ==
         nullptr);
  assert(pjrtx_mlx_metal_buffer_add_u8(nullptr, nullptr) == nullptr);
  assert(pjrtx_mlx_metal_buffer_binary_u8(
             nullptr, nullptr, PJRTX_MLX_METAL_U8_BINARY_ADD) == nullptr);
  assert(pjrtx_mlx_metal_buffer_unary_u8(
             nullptr, PJRTX_MLX_METAL_U8_UNARY_NEGATE) == nullptr);
  return 0;
}

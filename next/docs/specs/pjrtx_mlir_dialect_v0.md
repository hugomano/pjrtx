...
be implemented in small steps under `//next/pjrtx/compiler/...`.
...
//next/pjrtx/compiler:compiler
  consumes //next/pjrtx/compiler/mlir_state

//next/pjrtx/vertical_slice:mlir_state_tests
...
The first historical code slice now lives in `//next/pjrtx/compiler/mlir_state`. The
next refactor should move session ownership to `//next/pjrtx/mlir:session` and keep
`//next/pjrtx/compiler/mlir_state` focused on the temporary attribute bridge until
...
`//next/pjrtx/vertical_slice:mlir_state_test` keeps the first runfiles-backed
...
`//next/pjrtx/backend:mlir_bridge` extends the state machine to backend executable
calls without making `//next/pjrtx/compiler` depend on `//next/pjrtx/backend`. Backend
...
`//next/pjrtx/runtime:mlir_bridge` commits runtime allocator reservations after the

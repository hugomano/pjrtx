...
`//pjrtx/...`. They intentionally live beside the current README/bootstrap code
...
- segregated package ownership: no central `//pjrtx/core` bucket for new
  architecture code
...
1. Establish package boundaries from `package_boundaries_v0.md`: target,
   ...
   plugin packages, with no new dependency on `//pjrtx/core`.
2. Move existing bootstrap records out of `//pjrtx/core` by ownership while
   keeping tests green after each move.
3. Add report writer and normalization tests under `//pjrtx/report`.

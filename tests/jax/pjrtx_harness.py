...
def assert_backend_trace(name: str, trace_text: str, *, expect_compile: bool) -> None:
    ...
    for trace in compile_traces:
        if trace_int(trace, "backend_executable") != 1 or trace_int(trace, "unlowered") != 0:
            raise AssertionError(f"{name}: compile left the backend path: {trace}")
    for trace in execute_traces:
        if trace_int(trace, "backend_candidate") != 1:
            raise AssertionError(f"{name}: execute used a non-backend path: {trace}")
        if trace_int(trace, "backend_completion_pending") != 0:
            raise AssertionError(f"{name}: backend completion stayed pending: {trace}")

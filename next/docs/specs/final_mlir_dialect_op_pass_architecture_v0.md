...
flowchart TD
  ...
  mlir_session["//next/pjrtx/mlir session"]
  compiler["//next/pjrtx/compiler orchestration"]
...
- `//next/pjrtx/dialects` exists with TableGen/C++ skeleton files and Bazel targets
- `//next/pjrtx/mlir` owns `MlirSession` instead of `//next/pjrtx/compiler`
- one current attribute fact is replaced by a real dialect attr or op
- one Zig external pass is moved under `//next/pjrtx/compiler/passes`
